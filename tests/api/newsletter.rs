...
async fn newsletters_return_422_for_invalid_data() {
    ...
    for (invalid_body, error_message) in test_cases {
        let resposnse = app.post_newsletter(invalid_body).await;

        // Assert
        assert_eq!(
            422,
            resposnse.status().as_u16(),
...
async fn newsletters_are_delivered_to_confirmed_subscribers() {
    ...
    let newsletter_request_body = serde_json::json!({
        ...
        "content": {
            "text": "Newsletter body as plain text",
            "html": "<p>Newsletter body as HMTL</p>",
        }
    });
...
/// Use public API of the application under test to create
...
async fn create_unconfirmed_subscriber(app: &TestApp) -> ConfirmationLinks {
    ...
    // to retrieve confirmation link and return it
...
async fn create_confirmed_subscriber(app: &TestApp) {
    // We can reuse the sampler helper and just add
